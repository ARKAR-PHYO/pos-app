...
<div class="form-row">
    ...
    <div class="form-group col-md-6">
        ...
        <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" id="type" name="type" value="{{ old('type') }}" autofocus>
            @foreach ( $everlisttype as $type)
                <option value="{{ $type->id }}">{{ $type->everlisttype }}</option>
            @endforeach
        </select>
        @error('type')
        <span class="invalid-feedback" role="alert">
...
<div class="form-group">
    ...
    <textarea name="address" id="address" cols="3" rows="3" class="form-control @error('address') is-invalid @enderror" autofocus>{{ old('address') }}</textarea>
    @error('address')
    <span class="invalid-feedback" role="alert">
        <strong>{{ $message }}</strong>
    </span>
